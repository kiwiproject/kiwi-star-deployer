...
func Execute(w io.Writer, stages [][]plan.Entry, ws *workspace.Workspace, r runner.Runner, logBaseDir string) error {
	...
	for i, stage := range stages {
		...
		results := releaseStage(stage, ws, r, logDir)
...
// the input entries.
func releaseStage(entries []plan.Entry, ws *workspace.Workspace, r runner.Runner, logDir string) []libraryResult {
	...
	for i, entry := range entries {
		...
		go func(idx int, e plan.Entry) {
			defer wg.Done()
			results[idx] = releaseLibrary(e, ws, r, logDir)
		}(i, entry)
	}
...
func releaseLibrary(entry plan.Entry, ws *workspace.Workspace, r runner.Runner, logDir string) libraryResult {
	...
	if _, err := r.Run(runner.Options{
		...
		return libraryResult{name: entry.Name, logFile: logFile, output: buf.String(), err: fmt.Errorf("mvn clean: %w", err)}
	}
